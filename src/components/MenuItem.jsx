...
export default function MenuItem({ id, name, price, isCombo = false, comboData = null, index = 0, tooltip = '', soldOut = false, onComboAdd }) {
  ...
  return (
    <div
      className={`
        ...
        ${!soldOut && 'hover:-translate-y-1 hover:shadow-lg hover:shadow-peach-300/20'}
        ${tapped && !soldOut ? 'scale-95' : ''}
      `}
      style={{ animationDelay: `${index * 80}ms` }}
      onMouseDown={() => !soldOut && setTapped(true)}
      onMouseUp={() => setTapped(false)}
      onMouseLeave={() => setTapped(false)}
    >
      {/* Shine sweep on hover (disabled when sold out) */}
      {!soldOut && (
      ...
      <div className="flex items-center gap-2 relative z-10 min-w-0 flex-1">
        ...
        <span className={`
          font-[Inter] transition-colors duration-300 truncate
          ${soldOut
        ...
        {soldOut && (
          <span className="text-[9px] font-[Fredoka] text-white bg-warm-red px-1.5 py-0.5 rounded-full uppercase tracking-wider font-bold shrink-0">
            Sold Out
          </span>
        )}
        {isCombo && !soldOut && (
          <span className="text-[9px] font-[Fredoka] text-warm-gold bg-warm-gold/10 px-1.5 py-0.5 rounded-full uppercase tracking-wider shrink-0 hidden sm:inline-block">
            Best Value
          </span>
        )}
      </div>
      ...
      {/* Price */}
      <div className={`
        relative z-10 font-[Fredoka] font-bold transition-all duration-300 shrink-0 mx-2
        ${soldOut
